# ================================================================
# 04_noncyc_network.R
# Build a NONCYC pool by taking baseline road classes (from 2023 lines)
...
#   {city_tag}_{ver15}_{ver23}_noncyc_work_tol{tol_m}m_min{min_len}m.rds
#   {city_tag}_{ver15}_{ver23}_general_noncyc_tol{tol_m}m_min{min_len}m.rds
#   (optionally reuses or writes CI buffers:)
#   {city_tag}_{ver15}_buf_tol{tol_m}m.rds
#   {city_tag}_{ver23}_buf_tol{tol_m}m.rds
#
# Objects (in memory):
#   noncyc1523, general1523_n, buf15, buf23, buf_any
# ================================================================
...
# ---- cache paths ------------------------------------------------------------
rds_noncyc_1523  <- file.path(proc_dir, sprintf("%s_%s_%s_noncyc_work_tol%sm_min%sm.rds",
                                               city_tag, ver15, ver23, tol_m, min_len))
rds_noncycW_1523 <- file.path(proc_dir, sprintf("%s_%s_%s_general_noncyc_tol%sm_min%sm.rds",
                                               city_tag, ver15, ver23, tol_m, min_len))
...
# ---- FAST PATH --------------------------------------------------------------
if (reuse_cached && file.exists(rds_noncyc_1523) && file.exists(rds_noncycW_1523)) {
  noncyc1523     <- readRDS(rds_noncyc_1523)
  general1523_n <- readRDS(rds_noncycW_1523)
  message(
    "Non-CI 2015&2023 (loaded) — rows: ", nrow(noncyc1523),
    " | km: ", round(sum(as.numeric(st_length(noncyc1523)))/1000, 1)
  )
} else {
  ...
  if (!inherits(base23, "sf") || nrow(base23) == 0) {
    stop(sprintf(
      "04_noncyc_network: base23 is empty or not sf after filtering. Check base_roads vs l23$highway and snapshot/input. n=%s",
  ...
  noncyc1523 <- dplyr::bind_rows(keep_any, cut_any)
  if (nrow(noncyc1523) && min_len > 0) {
    noncyc1523 <- noncyc1523[as.numeric(sf::st_length(noncyc1523)) >= min_len, , drop = FALSE]
  }
  
  # ---- 4) WGS84 for mapping -------------------------------------------------
  general1523_n <- noncyc1523 |>
    sf::st_transform(crs_wgs) |>
    sf::st_make_valid() |>
    sf::st_simplify(dTolerance = 2)
  
  # ---- 5) Save caches -------------------------------------------------------
  try(saveRDS(noncyc1523,     rds_noncyc_1523),  silent = TRUE)
  try(saveRDS(general1523_n, rds_noncycW_1523), silent = TRUE)
  
  # ---- 6) Log ---------------------------------------------------------------
  message(
    "Non-CI 2015&2023 built — rows: ", nrow(noncyc1523),
    " | km: ", round(sum(as.numeric(sf::st_length(noncyc1523)))/1000, 1)
  )
}