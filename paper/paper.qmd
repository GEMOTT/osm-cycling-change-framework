---
title: "Validating OpenStreetMap for detecting cycling-infrastructure change: A reproducible workflow in Barcelona using Google Street View (2015–2023)"
# author:
...
abstract: |
 Cities are expanding their cycling networks rapidly, yet longitudinal data describing how these networks evolve remain limited. Volunteered Geographic Information, particularly OpenStreetMap (OSM), offers time-stamped spatial data on urban infrastructure, but its temporal reliability for detecting cycling-infrastructure change is uncertain. Therefore, independent validation is required to ensure that OSM-based estimates do not misrepresent the magnitude of change or bias longitudinal urban analyses. This study evaluates OSM’s accuracy in identifying additions and removals of cycling infrastructure in Barcelona between 2015 and 2023, using Google Street View (GSV) imagery as reference data within a reproducible, open-source workflow.

 Baseline and follow-up cycling networks were reconstructed from dated OSM snapshots and compared using geometric differencing to detect apparent additions and removals. A stratified sample of locations was independently verified using GSV to estimate precision, recall, and F1 scores. OSM-derived estimates indicate an 80.3% increase in cycling-infrastructure length (130.7 km to 235.7 km). Validation showed high recall for both additions (0.96) and removals (1.00), suggesting that OSM successfully captures most real-world changes. However, precision was strong for additions (0.82) but very low for removals (0.20), indicating that many apparent losses do not correspond to observed physical infrastructure change but rather to mapping updates. By quantifying precision and recall, this study provides a transferable framework for calibrating OSM-based measures of cycling-infrastructure change in longitudinal urban research.
...
This uncertainty in timing can influence the results of analyses that use historical OSM snapshots as evidence of change [@schmidl_approach_2021; @juhasz_towards_2021].
...
This study addresses this gap by introducing a reproducible, open-source workflow implemented in R to (i) construct historical cycling-infrastructure networks from dated OSM snapshots, (ii) detect additions and removals through geometric differencing, and (iii) validate OSM-inferred changes using a stratified GSV protocol in Barcelona (2015–2023). This work was conducted within the ERC-funded ATRAPA project (The Active Travel Backlash Paradox), which examines public responses to built-environment sustainable-travel interventions across European cities. [@gemott_research_group_atrapa_2025].
