...
\color{blue}The robustness of results to key geometric parameter choices (buffer distance, realignment threshold, and minimum segment length) was assessed through dedicated sensitivity analyses (see @sec-network-change and Supplement [S1](#s1-sensitivity)).\color{black}
...
\color{blue}As an external consistency check, we compared OSM-derived cycling-infrastructure network length with official municipal data for Barcelona [@amb2015_carril_bici; @bcn2023_carril_bici], one of the few cases where consistent longitudinal records are available. Both sources show a remarkably similar increase over the study period (OSM: 130.7 to 235.7 km; official: 133.0 to 234.0 km), with only minimal differences in aggregate network length at either time point. This close correspondence suggests that OSM-derived estimates capture the overall growth trajectory at the aggregate level. \color{black}
...
Finally, Barcelona likely represents a relatively well-mapped OSM context, given the concentration of OSM contributors and higher mapped completeness in many European and high-income urban areas; future research should assess how this workflow performs in lower-activity contexts and where mapping practices and tag usage may evolve differently [@herfort_spatio-temporal_2023].
...
By producing quantitative accuracy estimates within a transparent \color{blue}workflow with reproducible computational components and structured manual validation\color{black}, this study provides a practical basis for calibrating OSM-derived measures of cycling-infrastructure change and for accounting for measurement error in longitudinal evaluations. \color{blue}The transferability of the framework to other settings will depend on the availability of consistent historical OSM snapshots, suitable spatial units and associated data to support stratified sampling, and sufficient street-level imagery.\color{black}

More broadly, the findings support the responsible use of OSM and other VGI sources for urban change analysis, while underscoring the need to validate temporal change signals before drawing substantive policy or behavioural inferences.
