...
\color{blue}Building on this gap, this study aims to quantify the accuracy of OSM for detecting cycling-infrastructure change. \color{black} Using Barcelona over the 2015–2023 period as a case study, we implement a reproducible open-source workflow that combines longitudinal snapshot differencing with probability-based stratified validation against historical GSV imagery. The selected time window aligns with a broader research project examining infrastructure change across policy-relevant governance periods, although the present paper focuses exclusively on methodological validation. Detection performance is evaluated using precision, recall, and F1 scores. By integrating change detection and external validation within a transparent \color{blue}workflow with fully reproducible computational components and structured manual validation, the study provides a \color{black} framework for calibrating OSM-based measures of cycling-infrastructure change.
...
\color{blue}To maintain a conservative and visually interpretable definition of cycling infrastructure for subsequent GSV-based validation, we excluded `cycleway=shared_lane` tags. These tags are often associated with less segregated or more ambiguously defined facilities, such as advisory lane markings or shared-traffic treatments, which may not be consistently identifiable in historical GSV imagery. Including these tags would substantially broaden the definition of cycling infrastructure and could reduce consistency in the manual validation process.\color{black}
...
\color{blue}The robustness of key geometric parameters used in this step (buffer distance, realignment threshold, and minimum segment length) was assessed through dedicated sensitivity analyses (see @sec-network-change and Supplement [S1](#s1-sensitivity)).\color{black}
...
The results indicate that OSM-detected additions are generally reliable \color{blue}in this context, with high recall and good precision suggesting that most network expansions are captured. In contrast, apparent removals identified through snapshot differencing often do not correspond to on-street cycling-infrastructure loss; however, this evidence is based on a very small number of validated cases (n = 5) and is therefore subject to substantial uncertainty. These findings suggest that OSM histories can be informative for tracking network expansion, while inferred removals should be interpreted cautiously and, where possible, verified independently.\color{black}

By producing quantitative accuracy estimates within a transparent \color{blue}workflow with reproducible computational components and structured manual validation\color{black}, this study provides a practical basis for calibrating OSM-derived measures of cycling-infrastructure change and for accounting for measurement error in longitudinal evaluations. \color{blue}The transferability of the framework to other settings will depend on the availability of historical OSM snapshots, suitable spatial units to support stratified sampling, and sufficient street-level imagery.\color{black}
...
\color{blue}Provides the results of sensitivity checks for key geometric parameters used in the OSM change-detection workflow, including the differencing buffer, realignment threshold, and minimum segment length.\color{black}

\color{blue}The table reports the total length of detected additions, removals, and net change under alternative parameter settings, along with deviations from the baseline configuration.\color{black}

\color{blue}The baseline scenario corresponds to the default parameter configuration used in the analysis (10 m differencing buffer, 15 m realignment threshold, and 10 m minimum segment length). Values are computed on the filtered evaluation dataset (after excluding short fragments and likely realignments) and therefore differ from the raw network-level change estimates reported in tbl-consistency.\color{black}
