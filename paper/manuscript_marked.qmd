...
The analysis followed a five-step reproducible workflow (@fig-workflow) with two main components: (i) temporal differencing of dated OSM extracts to detect additions and removals of cycling infrastructure **(Steps 1–2)**, and (ii) stratified GSV validation to check a sample of these detected changes **(Steps 3–5)**.
...
**Throughout the paper, we use "cycling infrastructure" as an umbrella term to refer to these facilities, including both dedicated cycleways and on-road infrastructure encoded through cycleway-related tags.**
...
#### Stratified GSV validation of OSM-detected changes

**This second component of the workflow evaluates the OSM-detected changes identified in Steps 1–2.**
...
**As an external consistency check, we compared OSM-derived network length over time with official municipal data for Barcelona. OSM estimated an increase from 130.7 km to 235.7 km between 2015 and 2023, while the official data show an increase from \[X\] km to \[Y\] km over the comparable period. This broadly similar temporal evolution suggests that the OSM-derived estimates capture the overall growth pattern observed in the official record. This comparison is used as an aggregate consistency check, while the GSV-based validation remains the basis for assessing whether detected additions and removals correspond to visible on-street changes.**
...
Of the 105 sampled sites (39 additions, 6 removals and 50 non-cycling control sites), 86 (82%) provided usable GSV panoramas for both the baseline and follow-up periods, yielding 33 additions, 5 removals and 48 non-cycling control sites for analysis (@tbl-summary_stratum). **These validation points were drawn from a total of [N] OSM-detected change segments ([N_add] additions and [N_remove] removals), corresponding to approximately [X%] of all detected change candidates. The final sample size reflects the availability of eligible change segments within sampled tracts and the requirement for valid GSV imagery in both reference periods.** These validation points were distributed across all density–centrality strata, reflecting the stratified sampling design and the spatial availability of candidate change segments within the sampled tracts.
...
**Finally, while the proposed workflow is conceptually scalable, its application to much larger validation samples (e.g., thousands of sites) would face practical constraints. The validation relies on manual inspection of historical GSV imagery, which is time-intensive to scale. Importantly, currently available programmatic access options do not provide equivalent access to historical Street View imagery across time points, which limits the feasibility of fully automated large-scale validation. These constraints should be considered when extending the approach to larger study areas or higher-resolution validation designs.**
