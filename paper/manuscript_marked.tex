...
cycling infrastructure \textbf{(Steps 1--2)}, and (ii) stratified GSV
validation to check a sample of these detected changes \textbf{(Steps
3--5)}.
...
\textbf{Throughout the paper, we use ``cycling infrastructure'' as an
umbrella term to refer to these facilities, including both dedicated
cycleways and on-road infrastructure encoded through cycleway-related
tags.}
...
\subsubsection{Stratified GSV validation of OSM-detected
changes}\label{stratified-gsv-validation-of-osm-detected-changes}

\textbf{This second component of the workflow evaluates the OSM-detected
changes identified in Steps 1--2.}
...
(Table~\ref{tbl-summary_stratum}). \textbf{These validation points were
drawn from a total of {[}N{]} OSM-detected change segments ({[}N\_add{]}
additions and {[}N\_remove{]} removals), corresponding to approximately
{[}X\%{]} of all detected change candidates. The final sample size
reflects the availability of eligible change segments within sampled
tracts and the requirement for valid GSV imagery in both reference
...
tracts, the final allocation is constrained by the availability of
eligible change segments and usable GSV imagery. As a result, the
validation sample reflects a structured but feasibility-constrained
allocation rather than a strictly proportional or representative subset
of all detected changes. This leads to imbalances across classes,
particularly for removals, and implies that the reported metrics should
be interpreted as conditional on the sampled validation set rather than
as population-level estimates.}
...
reference year. This design provides a clearer non-cycling comparison
pool, but it may reduce the likelihood of identifying false negatives
near already mapped cycling facilities. As a result, recall estimates
...
Sixth, the core cycling‑infrastructure filter is intended to be broadly
applicable to other cities, but OSM's tagging conventions are
community‑driven and can vary across space and time. As a
volunteer-driven folksonomy rather than a strictly enforced ontology,
...
project's history (Mocnik et al. 2017). For example, painted or shared
lanes may have been tagged differently in earlier years or in different
cities. A narrow filter that only includes strictly dedicated cycleways
may miss valid infrastructure such as painted lanes on streets
(\texttt{cycleway=shared\_lane}), while a broader filter that includes
such tags could also count streets without proper \textbf{dedicated
cycling facilities}, resulting in minor under- or overestimation of
infrastructure. These temporal and spatial tagging differences introduce
some uncertainty in how well a universal filter captures all relevant
cycling infrastructure across different urban contexts.
