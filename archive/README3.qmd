...
samples_final_combined %>%
  ...
  # Identify tracts that are missing any valid osm_general GSV ≥ 2023
missing_general_tracts <- samples_all_wgs %>%
  filter(source == "osm_general") %>%
  mutate(gsv_year = as.integer(substr(gsv_date, 1, 4))) %>%
  group_by(tract_id) %>%
  summarise(has_recent = any(gsv_year >= 2023, na.rm = TRUE), .groups = "drop") %>%
  filter(!has_recent) %>%
  pull(tract_id)
  
  # All tract IDs with osm_general infrastructure
...
st_write(samples_final_combined, "data/validation/sample_points_with_links.geojson", delete_dsn = TRUE)

samples_df <- samples_final_combined %>%
  mutate(
