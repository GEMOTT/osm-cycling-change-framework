...
if (!dir.exists("wp1data/street-level")) {
  dir.create("wp1data/street-level")
}
...
for (english_name in names(cities)) {
  ...
  try({
    ...
    city_dir <- paste0("wp1data/street-level/", tolower(english_name))
...
for (english_name in names(cities)) {
  ...
  try({
    ...
    perimeter_path <- paste0("wp1data/street-level/", tolower(english_name), "/", tolower(english_name), "_perimeter.gpkg")
...
for (city in names(cities)) {
  ...
  # Define file paths
  perimeter_path <- paste0("wp1data/street-level/", tolower(city), "/", tolower(city), "_perimeter.gpkg")
  lines_170101_path <- paste0("wp1data/street-level/", tolower(city), "/", tolower(city), "_170101_lines.gpkg")
  lines_240101_path <- paste0("wp1data/street-level/", tolower(city), "/", tolower(city), "_240101_lines.gpkg")
...
barcelona_perimeter <- st_read("wp1data/street-level/barcelona/raw/0301040100_TermeMunicipal_UNITATS_ADM.shp")

# Define the file path to the downloaded .osm.pbf file
# local_osm_file <- "wp1data/street-level/milan/nord-ovest-170101.osm.pbf"  # Replace with actual path
...
# Save data
st_write(barcelona_160101_lines, "wp1data/street-level/barcelona/barcelona_160101_lines.gpkg", append=TRUE)
st_write(barcelona_200101_lines, "wp1data/street-level/barcelona/barcelona_200101_lines.gpkg", append=TRUE)
st_write(barcelona_lines_240101, "wp1data/street-level/barcelona/barcelona_240101_lines.gpkg", append=TRUE)
```
...
city <- "barcelona"
perimeter_path <- paste0("wp1data/street-level/", city, "/", city, "_perimeter.gpkg")
lines_160101_path <- paste0("wp1data/street-level/", city, "/", city, "_160101_lines.gpkg")
lines_200101_path <- paste0("wp1data/street-level/", city, "/", city, "_200101_lines.gpkg")
lines_240101_path <- paste0("wp1data/street-level/", city, "/", city, "_240101_lines.gpkg")
...
barcelona_tracts <- st_read("wp1data/street-level/barcelona/barcelona_census_tracts.gpkg", quiet = TRUE)

barcelona_network_2015 <- st_read("wp1data/street-level/barcelona/barcelona_160101_lines.gpkg", quiet = TRUE)
barcelona_network_2019 <- st_read("wp1data/street-level/barcelona/barcelona_200101_lines.gpkg", quiet = TRUE)
barcelona_network_2023 <- st_read("wp1data/street-level/barcelona/barcelona_240101_lines.gpkg", quiet = TRUE)
...
# Save the data
st_write(barcelona_tracts_with_all_ratios, "wp1data/area-level/barcelona/barcelona_indicators.gpkg", append = FALSE)
```
