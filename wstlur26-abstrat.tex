...
\hypersetup{
  ...
  pdfauthor={Eugeni Vidal-Tortosa; Oriol Marquet},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
(Buehler and Pucher, 2021; Szell et al., 2022). Yet researchers tend to
lack longitudinal datasets that capture where and when such
infrastructure changes occur. Official inventories are rarely maintained
consistently, while local field audits, though accurate, are costly and
...
to real-world conditions. The work forms part of the ATRAPA project on
Active Travel Transformations and Acceptability, which develops
comparative analyses of sustainable-mobility interventions across
European cities (GEMOTT Research Group, 2025).

\subsection{Data and methods}\label{data-and-methods}

The analysis followed a five-step reproducible workflow
(Figure~\ref{fig-workflow}), combining OSM temporal differencing with
stratified GSV validation. All stages were implemented in R using
open-source packages from the \emph{sf}, \emph{osmextract}, and
\emph{tidyverse} ecosystems.

\begin{figure}

\hfill\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figs/osm_gsv_flowchart.png}
...
Two OSM snapshots -- dated 1 January 2016 and 1 January 2024,
approximating the 2015 and 2023 situations -- were used to construct
baseline and follow-up cycling networks. Streets tagged as cycleways or
otherwise designated for bicycle use were classified as cycling
infrastructure, while all other urban roads were treated as non-cycling
infrastructure.

After data cleaning and projection, the two cycling network layers were
compared geometrically to identify changes over time. Segments present
in 2023 but not in 2015 were considered additions, while those
disappearing between years were removals. Minor positional or tagging
inconsistencies can create artificial differences, so small realignments
were identified and excluded from evaluation.

To ensure spatial representativeness, Barcelona's 1,063 census tracts
were cross-classified by population density and centrality into nine
strata. Six tracts were randomly selected within each stratum, yielding
54 tracts overall. Within them, up to two added, two removed, and one
non-cycling segment were sampled. The midpoint of each segment served as
a validation point and was linked to the nearest available GSV panorama.

Each sampled point was visually inspected by two trained coders for two
anchor periods: around 2015 (baseline) and 2023 (follow-up). When
imagery for the target year was unavailable or obscured, neighbouring
years were consulted in a fixed order (2023 → 2024 → 2022 for follow-up;
2015 → 2014 → 2016 for baseline) until a clear view was obtained. The
coding rule was simple: 1 if cycling infrastructure was visible, 0 if
clearly absent, and NA if indeterminate. Independent coding yielded a
high level of agreement: in 91 of 95 usable cases (95.8 \%), both coders
assigned the same code. All disagreements were jointly reviewed in the
final contradiction check and resolved by consensus.

Validation accuracy was assessed through standard metrics: precision
(share of OSM-detected changes that were real), recall (share of real
changes correctly captured by OSM), and the F1 score (harmonic mean of
precision and recall) were computed, with 95 \% Wilson confidence
intervals (CI) for each measure.
...
\% of the new network length. Removals were much less common and tended
to occur in intermediate-density areas.

\begin{figure}

\centering{

\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figs/infra_change_static.png}
...
Out of 105 sampled sites, 95 (90 \%) provided usable GSV imagery,
including 44 OSM-detected additions, 7 removals, and 44 non-cycling
...
cycling-infrastructure changes (2015--2023).}
...
\% CI {[}0.90--1.00{]}) but also includes some false positives
(precision \(\approx\) 0.77). For removals, recall was likewise 1.00 but
with a wide confidence interval due to the small sample size, while
precision drops sharply to around 0.29, meaning that most apparent
deletions do not correspond to true infrastructure loss. Overall
precision across all changes is 0.71 with an F1 of 0.83.
...
stratified sampling, and visual inspection -- is reproducible in any
city with sufficient Street View coverage. It underpins the ATRAPA Built
...
This research forms part of the ATRAPA project, which has received
funding from the European Union (ERC, Horizon Europe, grant agreement
No.~101117700) and from the Spanish Ministry of Science, Innovation and
Universities (Ministerio de Ciencia, Innovación y Universidades, grant
PID2022-136314OB-I00).

The authors gratefully acknowledge Víctor González Parra for his
collaboration as second coder during the Google Street View validation,
and thank the OpenStreetMap community for their invaluable contributions
