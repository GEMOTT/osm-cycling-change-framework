...
gen_wgs <- prep_pts(nonci_pts)
pers_wgs <- prep_pts(ci_stable_pts)
...
if (exists("tracts") && inherits(tracts,"sf") && nrow(tracts)) {
  m <- m %>% addPolygons(
    ...
    group      = "Tracts",
    color      = tract_stroke,   # "#000000"
    weight     = 1,
    opacity    = 0.7,
    fillColor  = tract_fill,     # "#FFFFFF"
    fillOpacity= 0.05,
    smoothFactor = 0.5,
...
# ---- NONCI (non-CI) network from chunk 10 ----------------------------------
...
if (!is.null(pers_wgs) && nrow(pers_wgs) > 0)
  m <- m %>% addCircleMarkers(
    ...
    radius = 4, weight = 1, color = stroke, fillColor = col_pers_pt, fillOpacity = 0.95,
    popup = ~popup_html
  )
...
m <- m %>%
  ...
  addLegend(
    ...
    colors = c(col_add_pt, col_rem_pt, col_gen_pt, col_pers_pt),   # ← add col_pers_pt
    labels = c("ADD points", "REMOVE points", "NONCI points", "CI_STABLE points"),
    opacity = 1
  )
m
```
...
ci_static_src <- if (exists("ci_static_pts")) ci_static_pts else if (exists("ci_stable_pts")) ci_stable_pts else NULL
...
sta_wgs <- if (!is.null(ci_static_src) && nrow(ci_static_src) > 0) as_wgs(ci_static_src) else NULL
...
pts_list <- list(
  ...
  if (!is.null(sta_wgs)) dplyr::mutate(sta_wgs, class = "CI_STATIC")
)
pts_all <- do.call(dplyr::bind_rows, pts_list)
if (!is.null(pts_all) && nrow(pts_all) > 0) {
  pts_all$class <- factor(pts_all$class, levels = c("ADD","REMOVE","NONCI","CI_STATIC"))
}
...
# colours (+ CI_STATIC)
cols <- c(ADD = "#E6AB02", REMOVE = "#D95F02", NONCI = "#1B9E77", CI_STATIC = "#3B82F6")
