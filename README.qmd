...
### S1. Interactive stratification map (density × centrality, 3×3) with sampled tracts
...
# ensure bivariate class exists (same as your ggplot version)
bt <- barcelona_tracts |> dplyr::mutate(centrality_flipped = -dist_centre_km)
bb_bivar <- biscale::bi_class(bt, x = dens_2022, y = centrality_flipped, style = "quantile", dim = 3)

# palette keyed to bi_class
pal_vec <- biscale::bi_pal(pal = "DkBlue2", dim = 3, preview = FALSE)
classes <- sort(unique(bb_bivar$bi_class))
col_map <- setNames(pal_vec, classes)

# labels
lab <- sprintf(
  "<b>Tract:</b> %s<br/><b>Stratum:</b> D%d_C%d<br/><b>Density 2022:</b> %s /km²<br/><b>Centre dist:</b> %0.1f km",
  bb_bivar$CUSEC %||% "", bb_bivar$dens_stratum, bb_bivar$cent_stratum,
  scales::comma(bb_bivar$dens_2022, accuracy = 1), bb_bivar$dist_centre_km
)

# custom 3×3 legend HTML (x=density, y=centrality)
leg_html <- htmltools::HTML(sprintf('
<div style="background:#fff;border:1px solid #ccc;border-radius:6px;padding:8px;">
  <div style="font-weight:600;margin-bottom:6px;">Density × Centrality</div>
  <div style="display:grid;grid-template-columns:repeat(3,16px);grid-gap:2px;margin-bottom:6px;">
    %s
  </div>
  <div style="display:flex;justify-content:space-between;font-size:11px;">
    <span>Low density</span><span>High density</span>
  </div>
  <div style="display:flex;align-items:center;justify-content:center;font-size:11px;margin-top:2px;">
    (↑ more central)
  </div>
</div>',
  paste(sprintf('<div style="width:16px;height:16px;background:%s"></div>', pal_vec), collapse = "")
))

# map
m_strata <- leaflet::leaflet(options = leaflet::leafletOptions(preferCanvas = TRUE)) |>
  leaflet::addProviderTiles("CartoDB.Positron") |>
  leaflet::addPolygons(
    data = sf::st_transform(bb_bivar, 4326), group = "Strata",
    fillColor = col_map[bb_bivar$bi_class], fillOpacity = 0.9,
    color = "#ffffff", weight = 0.5, opacity = 0.7,
    label = lapply(lab, htmltools::HTML),
    highlightOptions = leaflet::highlightOptions(weight = 2, color = "#000", bringToFront = TRUE)
  ) |>
  # sampled tracts outline
  leaflet::addPolygons(
    data = sf::st_transform(sampled_tracts, 4326), group = "Sampled tracts",
    fill = FALSE, color = "#000000", weight = 1.2, opacity = 0.9
  ) |>
  leaflet::addLayersControl(
    overlayGroups = c("Strata","Sampled tracts"),
    options = leaflet::layersControlOptions(collapsed = TRUE)
  ) |>
  leaflet::addControl(leg_html, position = "bottomright")

m_strata

```

### S2. Interactive validation points map (with GSV links)
...
### S3. Interactive infrastructure change map (2019→2023)
