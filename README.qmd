...
PERTRACT_REM  <- 4
PERTRACT_GEN  <- 2
FORCE_QUOTA   <- TRUE      # prevent duplicates
target_ratio  <- 0.6
...
# choose how much to scale
K_ADD <- 3; K_REM <- 3; K_GEN <- 2

added_pts   <- sample_points_along(added_by_tr,   k = K_ADD) %>% dplyr::mutate(class = "ADD")
removed_pts <- sample_points_along(removed_by_tr, k = K_REM) %>% dplyr::mutate(class = "REMOVE")
gen_pts     <- sample_points_along(general_by_tr, k = K_GEN) %>% dplyr::mutate(class = "GENERAL")
...
# --- Top-up to ensure stratum balance -----------------------------------------
...
added_pts    <- points_on_lines(added_by_tr)
removed_pts  <- points_on_lines(removed_by_tr)
gen_pts      <- points_on_lines(general_by_tr)
