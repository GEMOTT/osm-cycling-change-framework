...
**Aim**: Assess whether OSM reliably reflects new or removed bike lanes in Barcelona (2019–2023) using Google Street View (GSV) as ground truth, as a basis for scalable validation in other cities.
...
digraph {
  ...
  A [width=6, label=<
    <TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLPADDING=\"6\">
      ...
      <TR><TD>Download &amp; preprocess snapshots:<BR/>2019 → 2020-01-01 · 2023 → 2024-01-01</TD></TR>
    </TABLE>
  >]
  ...
  D [width=6.8, label=<
    <TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLPADDING=\"6\">
      ...
      <TR><TD>Windows: 2018–2019 · 2023–2024<BR/>Assign present/absent, verifiable → TP/FP/FN</TD></TR>
    </TABLE>
  >]
...
#| fig-cap: "Figure 1: Main workflow steps (2019–2023)."
#| fig-align: center
#| echo: false
...
- Snapshots: 2020-01-01 (≈2019) and 2024-01-01 (≈2023)

- Define Cycling Infrastructure (CI) network: `highway=cycleway`, `cycleway=*`, `cycleway:left/right/both=*`, `bicycle_road=yes`, or designated `path/footway/track` for bicycles.

<!-- + Normalise geometries (metric CRS, min-length filter, tolerance buffer) -->

- Define general network (NONCI): Base roads excluding all CI tags

##### 2. Calculate geometric differences

- **ADDED**: CI in 2023 but not 2019

- **REMOVED**: CI in 2019 but not 2023
...
# --- tolerance windows (inclusive) ------------------------------------------
win_2019 <- c(2018, 2019) 
win_2023 <- c(2023, 2024)  

# minimal writer (fixed order)
# minimal writer (nice view: frozen panes + table)
# minimal writer (adds tolerance-window banner; table + freeze panes)
write_one <- function(df, sheet, freeze_cols = 3){
  wanted <- c("id","class","tract_id","stratum","lon","lat","gsv_link","GSV", "present_2019","present_2023","notes")
  for (nm in setdiff(wanted, names(df))) df[[nm]] <- NA
  df <- df[, wanted, drop = FALSE]

  openxlsx::addWorksheet(wb, sheet, gridLines = TRUE, tabColour = switch(sheet,
    "ADD"="green","REMOVE"="red","NONCI"="orange","CI_STATIC"="blue","gray"))

  # Make GSV column wider
  # find the GSV column
# --- banner starting at the GSV column (from GSV → last col) ----------------
gsv_col  <- match("GSV", names(df))              # H in your current layout
startCol <- ifelse(is.na(gsv_col), 1L, gsv_col)  # fallback to A if GSV is missing

note <- sprintf("Tolerance windows — 2019: %d–%d · 2023: %d–%d",
                win_2019[1], win_2019[2], win_2023[1], win_2023[2])

sty  <- openxlsx::createStyle(
  fgFill = "#FFF7CC", halign = "left",
  textDecoration = "bold", fontColour = "#333333",
  wrapText = TRUE
)

openxlsx::writeData(wb, sheet, note, startRow = 1, startCol = startCol)
openxlsx::mergeCells(wb, sheet, cols = startCol:ncol(df), rows = 1)
openxlsx::addStyle(wb, sheet, style = sty,
                   rows = 1, cols = startCol:ncol(df),
                   gridExpand = TRUE, stack = TRUE)


  # --- write as a simple range (no Table, no filters) ---
  openxlsx::writeData(wb, sheet, df, startRow = 3, startCol = 1)

  # clickable GSV (first data row is row 4 now)
  if (nrow(df)) {
    openxlsx::writeFormula(
      wb, sheet,
      x = paste0('HYPERLINK("', df$gsv_link, '","Open GSV")'),
      startCol = which(names(df)=="GSV"), startRow = 4
    )
  }

  # widths (fix col 1 so banner doesn’t stretch it) + freeze header + key cols
  w <- rep("auto", ncol(df)); w[1] <- 12
  openxlsx::setColWidths(wb, sheet, cols = 1:ncol(df), widths = w)
  openxlsx::freezePane(wb, sheet, firstActiveRow = 4, firstActiveCol = freeze_cols + 1)

  # IMPORTANT: no addFilter() call here → no dropdowns
}
...
gen_tbl  <- mk_tbl(nonci_pts,     "NONCI", "NONCI")   # or "NONCI" if you prefer

# --- write -------------------------------------------------------
write_one(add_tbl, "ADD")
write_one(rem_tbl, "REMOVE")
write_one(gen_tbl, "NONCI")       # or "NONCI"
