...
suppressPackageStartupMessages({
  ...
  library(biscale); library(readxl); library(purrr); library(DiagrammeR); library(DiagrammeRsvg);
...
# k evenly-spaced points per (clipped) line — stays inside tracts
points_on_lines <- # Create k evenly spaced points per LINESTRING (safe on odd geoms)
sample_points_along <- function(x, k = 3){
  stopifnot(inherits(x, "sf"))
  if (!nrow(x) || k < 1) return(x[0, , drop = FALSE])

  g <- sf::st_geometry(x)
  crs_in <- sf::st_crs(x)

  # per-row sampling, always returning exactly k POINT sfgs
  pts_list <- lapply(seq_len(nrow(x)), function(i){
    gi <- sf::st_make_valid(g[i])
    ...
    if (inherits(gi1, "try-error") || !inherits(gi1, "sfc_LINESTRING")) {
      ...
      if (inherits(parts, "try-error") || length(parts) == 0) {
        # fall back to k duplicates of an empty point
        return(rep(list(sf::st_point()), k))
      }
      gi1 <- parts[which.max(as.numeric(sf::st_length(parts)))]
    }

    # k regular points; fallback to point_on_surface if needed
    p <- try(sf::st_line_sample(gi1, n = k, type = "regular"), silent = TRUE)
    if (inherits(p, "try-error") || length(p) == 0)
      p <- sf::st_point_on_surface(gi1)

    p <- suppressWarnings(sf::st_cast(p, "POINT"))

    # pad/truncate to exactly k sfgs
    sfgs <- if (length(p) >= k) {
      p[1:k]
    } else {
      c(p, rep(list(p[[length(p)]]), k - length(p)))
    }
    sfgs
  })

  # flatten and repeat attributes k times per row
  geom <- sf::st_sfc(unlist(pts_list, recursive = FALSE), crs = crs_in)
  attrs <- sf::st_drop_geometry(x)[rep(seq_len(nrow(x)), each = k), , drop = FALSE]
  sf::st_as_sf(attrs, geometry = geom)
}


# --- 1) Robust midpoint: exactly one point per line (no losses) ---------------
points_on_lines <- function(x){
  # Always return same row count as input
  ...
  pts <- lapply(seq_len(nrow(x)), function(i){
    gi <- g[i]
    if (sf::st_is_empty(gi)) return(sf::st_point())

    gi <- sf::st_make_valid(gi)

    # ensure a simple LINESTRING
    gi1 <- try(suppressWarnings(sf::st_line_merge(gi)), silent = TRUE)
    if (inherits(gi1, "try-error") || !inherits(gi1, "sfc_LINESTRING")) {
      parts <- try(suppressWarnings(sf::st_cast(gi, "LINESTRING")), silent = TRUE)
      if (inherits(parts, "try-error") || length(parts) == 0)
    ...
    # Try sampling midpoint; fallback to centroid/point_on_surface
...
# --- 2) One-liner: lines → points table with lon/lat + GSV link ---------------
...
coerce_for_bind <- function(df){
  chr <- c("class","interval","gsv_link","notes")
  num <- c("lon","lat")
...
# --- 5) Core sampler: clips lines inside each tract ---------------------------
...
# --- Top-up: fill deficits per (tract × stratum) without sf joins -------------
top_up <- function(candidates,
                   tracts,
                   picked,
                   min_per_stratum = 2,
  ...
  stopifnot(inherits(candidates, "sf"), inherits(tracts, "sf"))
  if (!inherits(picked, "sf")) {
    picked <- tracts[0, , drop = FALSE]
  }

  # Harmonize CRS
  if (sf::st_crs(candidates) != sf::st_crs(tracts))
    candidates <- sf::st_transform(candidates, sf::st_crs(tracts))
  if (sf::st_crs(picked) != sf::st_crs(tracts))
    picked <- sf::st_transform(picked, sf::st_crs(tracts))

  # Ensure LINESTRING candidates
  keep_lines <- sf::st_geometry_type(candidates, by_geometry = TRUE) %in% c("LINESTRING","MULTILINESTRING")
  cand <- candidates[keep_lines, , drop = FALSE]
  if (!nrow(cand)) return(picked)
  cand <- sf::st_make_valid(cand)
  cand <- suppressWarnings(sf::st_cast(cand, "LINESTRING"))
  cand <- cand[!sf::st_is_empty(sf::st_geometry(cand)), , drop = FALSE]

  # Data-only frames (no geometry) for grouping math
  counts_now <- sf::st_drop_geometry(picked) |>
    dplyr::count(.data[[tract_id_col]], .data[[stratum_col]], name = "n_now")

  all_groups <- sf::st_drop_geometry(tracts) |>
    dplyr::distinct(.data[[tract_id_col]], .data[[stratum_col]])

  needs <- all_groups |>
    dplyr::left_join(counts_now,
      by = setNames(c(tract_id_col, stratum_col), c(tract_id_col, stratum_col))
    ) |>
    dplyr::mutate(n_now = tidyr::replace_na(.data$n_now, 0L),
                  n_need = pmax(0L, min_per_stratum - .data$n_now)) |>
    dplyr::filter(.data$n_need > 0L)

  if (!nrow(needs)) return(picked)

  set.seed(seed)
  adds <- vector("list", nrow(needs))

  for (i in seq_len(nrow(needs))) {
    tr_id <- needs[[tract_id_col]][i]
    st_id <- needs[[stratum_col]][i]
    k     <- needs$n_need[i]

    # Single tract polygon for this group
    t_i <- tracts[tracts[[tract_id_col]] == tr_id & tracts[[stratum_col]] == st_id, , drop = FALSE]
    if (!nrow(t_i)) next

    # Intersect candidates with the tract polygon
    idx <- sf::st_intersects(t_i, cand)[[1]]
    if (!length(idx)) next
    clp <- suppressWarnings(sf::st_intersection(cand[idx, , drop = FALSE], t_i))
    clp <- suppressWarnings(sf::st_collection_extract(clp, "LINESTRING", warn = FALSE))
    clp <- clp[!sf::st_is_empty(sf::st_geometry(clp)), , drop = FALSE]
    if (!nrow(clp)) next

    # Length-weighted sampling; with replacement if pool < k
    lens <- as.numeric(sf::st_length(clp)); lens[!is.finite(lens)] <- 1
    prob <- lens / sum(lens)
    take <- if (nrow(clp) >= k) sample(seq_len(nrow(clp)), k, replace = FALSE, prob = prob)
            else                  sample(seq_len(nrow(clp)), k, replace = TRUE,  prob = prob)

    piece <- clp[take, , drop = FALSE]
    piece[[tract_id_col]] <- tr_id
    piece[[stratum_col]]  <- st_id
    adds[[i]] <- piece
  }

  adds_sf <- dplyr::bind_rows(adds)
  if (!nrow(adds_sf)) return(picked)

  dplyr::bind_rows(picked, adds_sf)
}
```
...
PERTRACT_REM  <- 4
PERTRACT_GEN  <- 2
FORCE_QUOTA   <- TRUE      # allow replacement if candidates < quota
target_ratio  <- 0.6

# --- Harmonise CRS and stratum -------------------------------------------------
...
# --- PERSISTENT cycle pool (19→23 overlap) ------------------------------------
if (!exists("persist") || !inherits(persist, "sf")) {
  cyc19_buf <- sf::st_buffer(sf::st_union(sf::st_geometry(cyc19_n)), tol_m)
  persist <- sf::st_intersection(sf::st_geometry(cyc23_n), cyc19_buf) |>
...
# GENERAL across tracts with changes; fallback to all tracts if none flagged
change_ids <- unique(na.omit(c(added_by_tr$tract_id, removed_by_tr$tract_id)))
tracts_for_general <- dplyr::filter(tracts, tract_id %in% change_ids)
...
# persistent selections (same per-tract quota as general)
persist_by_tr <- sample_lines_by_tract(persist, tracts, PERTRACT_GEN, replace = FORCE_QUOTA)

# --- Top-up to ensure stratum balance (lines) ----------------------------------
# (requires top_up() defined in helpers chunk)
min_gen_per_stratum <- 2
min_rem_per_stratum <- 3
general_by_tr <- top_up(noncycle23, tracts, general_by_tr, min_gen_per_stratum)
removed_by_tr <- top_up(removed,     tracts, removed_by_tr, min_rem_per_stratum)

# --- Convert selected LINES to ONE point per line ------------------------------
# (use points_on_lines(): robust midpoint, 1 point per line)
...
keep_in <- function(pts, tracts, eps = 0.5){
  ...
  if (sf::st_crs(pts) != sf::st_crs(tracts))
    pts <- sf::st_transform(pts, sf::st_crs(tracts))
...
# --- Tables with lon/lat -------------------------------------------------------
add_tbl <- to_lonlat_tbl(added_pts)   |> dplyr::mutate(interval="2019→2023", source="ADD_FLAG")     |> add_validation_cols() |> coerce_for_bind()
rem_tbl <- to_lonlat_tbl(removed_pts) |> dplyr::mutate(interval="2019→2023", source="REMOVE_FLAG")  |> add_validation_cols() |> coerce_for_bind()
gen_tbl <- dplyr::bind_rows(
  to_lonlat_tbl(gen_pts)     |> dplyr::mutate(interval="2019→2023", source="NONCYCLE_2023"),
  to_lonlat_tbl(persist_pts) |> dplyr::mutate(interval="2019→2023", source="PERSIST_19_23")
) |> add_validation_cols() |> coerce_for_bind()

# cap general to target ratio vs flagged
n_flagged   <- nrow(add_tbl) + nrow(rem_tbl)
max_general <- max(20, round(target_ratio * n_flagged))
if (nrow(gen_tbl) > max_general) {
  set.seed(123)
  gen_tbl <- gen_tbl[sample(seq_len(nrow(gen_tbl)), max_general), , drop = FALSE]
}
...
check_inside <- function(pts, tracts, eps = 0.5){
  sum(!apply(sf::st_is_within_distance(pts, tracts, dist = eps, sparse = FALSE), 1, any))
}
cat("\nPoints outside tract boundaries (should be 0):\n")
print(rbind(
  ADD     = check_inside(added_pts, tracts),
  REMOVE  = check_inside(removed_pts, tracts),
  GENERAL = check_inside(gen_pts, tracts),
  PERSIST = check_inside(persist_pts, tracts)
))

as.data.frame(table(stratum = added_by_tr$stratum))
as.data.frame(table(stratum = removed_by_tr$stratum))
as.data.frame(table(stratum = general_by_tr$stratum))
```
...
if (exists("tracts") && inherits(tracts,"sf") && nrow(tracts)) {
  m <- m %>%
    ...
    addPolygons(
      ...
      color = tract_stroke, opacity = 0.7, weight = 1.5,
