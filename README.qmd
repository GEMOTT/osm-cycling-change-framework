...
- It **does not** meet the CI rule above, **and**  
- `highway` ∈ `{primary, secondary, tertiary, unclassified, residential, primary_link, secondary_link, tertiary_link, living_street, pedestrian}`, **and**  
- It is **not**:
  - a shared path (`highway ∈ {path, footway, track}` with `bicycle ∈ {designated, yes}`), or  
  - a local/shared bikeway (`cycleway* ∈ {shared, designated}` without one of the CI rules above).
...
if (reuse_cached && file.exists(rds_nonci_1523) && file.exists(rds_nonciW_1523)) {
  ...
  # ---- 1) Base roads + exclude local/shared bikeways ------------------------
  # Shared paths are excluded by design because base_roads does not include path/footway/track
  base_roads <- c(
  ...
  # Exclude "local/shared bikeways": any cycleway* column in {shared, designated}
  if (nrow(base23)) {
    cyc_cols <- names(base23)[grepl("^cycleway", names(base23), ignore.case = TRUE)]
    if (length(cyc_cols)) {
      is_local_shared <- rep(FALSE, nrow(base23))
      for (col in cyc_cols) {
        v <- tolower(get_chr(base23, col))
        is_local_shared <- is_local_shared | (!is.na(v) & v %in% c("shared","designated"))
      }
      base23 <- base23[!is_local_shared, , drop = FALSE]
    }
  }

  stopifnot(inherits(base23,"sf"))
  if (!nrow(base23)) {
    warning("After excluding local/shared bikeways, base23 is empty.")
    nonci1523     <- base23
    general1523_n <- sf::st_transform(base23, 4326)
    try(saveRDS(nonci1523,     rds_nonci_1523),  silent = TRUE)
    try(saveRDS(general1523_n, rds_nonciW_1523), silent = TRUE)
    message("Non-CI 2015&2023 built — rows: 0 | km: 0.0")
  } else {
...
-   OSM captures almost all **additions** (high recall ≈ 0.97), but with some false positives (precision ≈ 0.77). For **removals**, OSM detects few true cases and produces many false removals (precision ≈ 0.25), despite near-perfect recall on the few true removals observed.
