...
# sampled_tracts <- sampled_tracts[1:3, ]
...
general_lines <- streets %>%
  filter(highway %in% c("primary", "secondary", "tertiary", "residential", "living_street", "service"))
...
samples_all <- purrr::map_dfr(1:nrow(sampled_tracts), function(i) {
  ...
  out1 <- sample_points_from_lines(cycleway_lines, geom, tract$tract_id, 2, "osm_cycleway")
  out2 <- sample_points_from_lines(general_lines, geom, tract$tract_id, 2, "osm_general")
...
tmap_save(final_plot, filename = "figs/final_map_interactive.html")
```
