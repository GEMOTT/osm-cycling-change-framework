...
#| eval: false
#| include: false
# Full R Code for Stratified Sampling by Density and Centrality
...
# ---- Load shapefile ----
shapefile_path <- "unitats_censals_2024.shp"
secciones <- st_read(shapefile_path, quiet = TRUE) %>%
  st_transform(25831)  # UTM projection for accurate distance/area

# ---- Compute population density ----
secciones <- secciones %>%
  mutate(area_km2 = set_units(st_area(.), "km^2"),
         density = POBLACIO / as.numeric(area_km2))

# ---- Define city center (adjust if needed) ----
# Example: Plaça Catalunya, Barcelona (in EPSG:25831)
centroide_centro <- st_sfc(st_point(c(431870, 4581450)), crs = 25831)

# ---- Compute distance to city center ----
secciones <- secciones %>%
  mutate(dist_centro_m = as.numeric(st_distance(st_centroid(geometry), centroide_centro)))

# ---- Classify into strata ----

# Population density strata (quartiles)
secciones <- secciones %>%
  mutate(stratum_density = ntile(density, 4))

# Centrality strata (e.g., near: Q1, mid: Q2–Q3, far: Q4)
secciones <- secciones %>%
  mutate(stratum_centrality = ntile(dist_centro_m, 4))

# Combine into joint stratum
secciones <- secciones %>%
  mutate(stratum_combined = paste0("D", stratum_density, "_C", stratum_centrality))

# ---- Stratified sampling ----
set.seed(123)
n_per_stratum <- 5  # adjust

sampled <- secciones %>%
  group_by(stratum_combined) %>%
  sample_n(size = min(n_per_stratum, n()), replace = FALSE) %>%
  ungroup()

# ---- Summary ----
table(sampled$stratum_combined)

# ---- Optional: Save for use in later scripts ----
st_write(sampled, "secciones_stratified_sample.geojson", delete_dsn = TRUE)
