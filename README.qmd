...
suppressPackageStartupMessages({
  ...
  library(biscale); library(readxl); library(purrr) 
...
if (file.exists(outfile)) {
  message("✔ Found existing coded Excel. Not overwriting: ", normalizePath(outfile))
} else {
  wb <- createWorkbook()

  write_one <- function(df, sheet){
    df2 <- df %>% mutate(GSV = NA_character_) %>% relocate(GSV, .after = gsv_link)
  ...
  message("→ Fill it, then re-run the metrics chunks (22 & 23).")
}
