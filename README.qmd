...
#| fig-cap: "Figure 1. Stratified sampling bivariate map (density × centrality, 3×3) with selected validation tracts; inset shows the bivariate legend."
#| fig-pos: 't'
#| echo: false
...
We evaluated OSM change detection against GSV imagery using three indicators:

- **True Positive (TP):** an OSM-flagged change (ADD or REMOVE) that is confirmed in GSV.  
- **False Positive (FP):** an OSM-flagged change not confirmed in GSV (e.g. late mapping in ADD, data clean-up in REMOVE).  
- **False Negative (FN):** a true change observed in GENERAL points that OSM failed to flag (missed ADD or REMOVE).  
...
- **Precision** is estimated from *OSM-flagged points only* (15 ADD, 5 REMOVE).  
- **False negatives (FN)** are detected in *GENERAL points* (20), which are not OSM-flagged.  
- **Recall** therefore combines both sources: OSM-flagged TPs in the numerator, and OSM-flagged TPs plus FN from GENERAL in the denominator.  

Overall performance metrics are reported separately for ADD and REMOVE, and pooled across classes.
...
#| echo: false
#| message: true        # <-- show diagnostics while we fix this
#| warning: false
...
# ---------- HELPERS ----------
# robust truthy/yes parser
is_yes <- function(x){
  x <- tolower(str_trim(as.character(x)))
  x %in% c("yes","y","true","1","sí","si")
}

# read + normalize, trimming column names (handles trailing spaces)
safe_read <- function(path, sh, need_cols = c("present_2019","present_2023","verifiable_2019","verifiable_2023","stratum")){
  if (!file.exists(path)) stop(sprintf("File not found: %s (wd: %s)", path, getwd()))
  df <- read_xlsx(path, sheet = sh)
  # Trim/clean column names (catch hidden spaces)
  names(df) <- trimws(names(df))
  # Diagnostics
  message(sprintf("Loaded sheet '%s': n=%s | cols=%s", sh, nrow(df), paste(names(df), collapse=", ")))
  # Check required columns exist (after trimming)
  if (!all(need_cols %in% names(df))) {
    missing <- setdiff(need_cols, names(df))
    stop(sprintf("Sheet '%s' is missing columns: %s", sh, paste(missing, collapse=", ")))
  }
  df %>%
    mutate(
...
# ---------- READ WORKBOOK DIRECTLY (no dependencies) ----------
message(sprintf("Reading workbook: %s", normalizePath(FILE, winslash = "/",
                                                     mustWork = FALSE)))
...
# ---------- BUILD USABLE UNIVERSE ----------
samples_tbl2 <- bind_rows(
...
message(sprintf("Sampled counts by class: %s",
                paste(capture.output(print(table(samples_tbl2$class), quote=FALSE)), collapse=" | ")))

usable_tbl <- samples_tbl2 %>%
  filter(verifiable_2019, verifiable_2023)   # both TRUE after is_yes()

# Diagnostics after filter
message(sprintf("Usable total n = %d (expected around 39)", nrow(usable_tbl)))
message(sprintf("Usable counts by class: %s",
                paste(capture.output(print(table(usable_tbl$class), quote=FALSE)), collapse=" | ")))

if (nrow(usable_tbl) == 0) {
  stop("Usable table is empty after filtering verifiable_2019 & verifiable_2023. 
Check that those columns contain recognizable yes/true values, and that we’re reading the correct file.")
}

# ---------- TABLE 3 (stratum × class) ----------
summary_stratum_class <- usable_tbl %>%
  filter(!is.na(stratum), !is.na(class)) %>%
...
summary_stratum_class_full <- bind_rows(
  ...
  summary_stratum_class %>%
    summarise(stratum = "TOTAL", across(where(is.numeric), sum, na.rm = TRUE))
)

# Final sanity check
tot_row <- summary_stratum_class_full %>% filter(stratum == "TOTAL") %>% pull(Total)
message(sprintf("Table 3 TOTAL = %d", tot_row))
...
```{r}
#| label: 24-validation-panels
#| echo: false
#| message: false
#| warning: false

library(readxl); library(dplyr); library(stringr); library(tidyr); library(tibble)
...
norm_yes <- function(x){
  ...
  yes <- c("yes","y","true","1","sí","si","s\u00ed")
  no  <- c("no","n","false","0")
  ifelse(y %in% yes, "yes", ifelse(y %in% no, "no", NA_character_))
}
safe_read <- function(path, sheet){
  ...
  df %>%
    mutate(
      present_2019    = tolower(str_trim(as.character(present_2019))),
      present_2023    = tolower(str_trim(as.character(present_2023))),
      verifiable_2019 = norm_yes(verifiable_2019),
      verifiable_2023 = norm_yes(verifiable_2023)
    )
}
metric <- function(tp, fp, fn){
...
# --- load & usable filters ---
add <- safe_read(FILE, "ADD");    add_v <- add %>% filter(verifiable_2019=="yes", verifiable_2023=="yes")
rem <- safe_read(FILE, "REMOVE"); rem_v <- rem %>% filter(verifiable_2019=="yes", verifiable_2023=="yes")
gen <- safe_read(FILE, "GENERAL");gen_v <- gen %>% filter(verifiable_2019=="yes", verifiable_2023=="yes")

# --- Panel A: OSM-flagged (for Precision) ---
TP_add <- sum(add_v$present_2019=="no"  & add_v$present_2023=="yes")
FP_add <- sum(add_v$present_2023=="no"  | (add_v$present_2019=="yes" & add_v$present_2023=="yes"))
TP_rem <- sum(rem_v$present_2019=="yes" & rem_v$present_2023=="no")
FP_rem <- sum((rem_v$present_2019=="no" & rem_v$present_2023=="no") |
              (rem_v$present_2019=="yes" & rem_v$present_2023=="yes"))

panelA <- tribble(
  ~Class,   ~`n (OSM-flagged, usable)`, ~TP, ~FP, ~Precision,
  "ADD",          nrow(add_v),           TP_add, FP_add, ifelse((TP_add+FP_add)>0, TP_add/(TP_add+FP_add), NA_real_),
  "REMOVE",       nrow(rem_v),           TP_rem, FP_rem, ifelse((TP_rem+FP_rem)>0, TP_rem/(TP_rem+FP_rem), NA_real_),
  "Pooled",       nrow(add_v)+nrow(rem_v), TP_add+TP_rem, FP_add+FP_rem,
                 ifelse((TP_add+TP_rem+FP_add+FP_rem)>0, (TP_add+TP_rem)/(TP_add+TP_rem+FP_add+FP_rem), NA_real_)
) %>% mutate(Precision = sprintf("%.2f", Precision))

knitr::kable(
  panelA,
  caption = "Panel A. OSM-flagged points (for Precision): 15 ADD + 5 REMOVE = 20 usable",
  align = c("l","r","r","r","r"),
  digits = 0
)

# --- Panel B: GENERAL points (for False Negatives) ---
FN_add <- sum(gen_v$present_2019=="no"  & gen_v$present_2023=="yes")
FN_rem <- sum(gen_v$present_2019=="yes" & gen_v$present_2023=="no")

panelB <- tribble(
  ~`GENERAL n (usable)`, ~`FN (missed ADD)`, ~`FN (missed REMOVE)`, ~`Total FN`,
  nrow(gen_v),            FN_add,             FN_rem,                FN_add + FN_rem
)

knitr::kable(
  panelB,
  caption = "Panel B. GENERAL points (for False Negatives detection)",
  align = c("r","r","r","r"),
  digits = 0
)

# --- Panel C: Final metrics (Precision from Panel A; Recall uses FN from Panel B) ---
m_add  <- metric(TP_add, FP_add, FN_add)
m_rem  <- metric(TP_rem, FP_rem, FN_rem)
m_pool <- metric(TP_add+TP_rem, FP_add+FP_rem, FN_add+FN_rem)

panelC <- tribble(
  ~Class,   ~`n (flagged, usable)`, ~TP, ~FP, ~FN, ~Precision,                 ~Recall,                    ~F1,
  "ADD",          nrow(add_v),      TP_add, FP_add, FN_add, sprintf("%.2f", m_add["p"]),  sprintf("%.2f", m_add["r"]),  sprintf("%.2f", m_add["f1"]),
  "REMOVE",       nrow(rem_v),      TP_rem, FP_rem, FN_rem, sprintf("%.2f", m_rem["p"]),  sprintf("%.2f", m_rem["r"]),  sprintf("%.2f", m_rem["f1"]),
  "Pooled", nrow(add_v)+nrow(rem_v),TP_add+TP_rem, FP_add+FP_rem, FN_add+FN_rem,
               sprintf("%.2f", m_pool["p"]), sprintf("%.2f", m_pool["r"]), sprintf("%.2f", m_pool["f1"])
)

knitr::kable(
  panelC,
  caption = "Panel C. Final metrics (Precision on OSM-flagged; Recall includes FN from GENERAL)",
