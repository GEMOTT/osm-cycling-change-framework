...
digraph {
  ...
  A [width=5, label=<
    <TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLPADDING=\"6\">
      ...
      <TR><TD>Download &amp; preprocess:<BR/>
   2015 (snapshot 2016-01-01)<BR/>
   2023 (snapshot 2024-01-01)</TD></TR>
    </TABLE>
  >]

  B [width=5, label=<
    <TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLPADDING=\"6\">
      <TR><TD><B>2) Geometric differencing</B></TD></TR>
      <TR><TD>Detect <I>ADD</I>/<I>REMOVE</I><BR/>
  between 2015 → 2023</TD></TR>
    </TABLE>
  >]
  ...
  C [width=6, label=<
    <TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLPADDING=\"6\">
      ...
      <TR><TD>Stratify by density × centrality (3×3);<BR/>
  length-weighted points within strata</TD></TR>
    </TABLE>
  >]

  D [width=7, label=<
    <TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLPADDING=\"6\">
      <TR><TD><B>4) GSV validation (inspect &amp; classify)</B></TD></TR>
      <TR><TD>Anchor years: 2015 &amp; 2023 (±1 year)<BR/>
  Code present/absent → TP/FP/FN</TD></TR>
    </TABLE>
  >]
  ...
  E [width=5, label=<
    <TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLPADDING=\"6\">
      ...
      <TR><TD>Compute Precision, Recall, F1</TD></TR>
    </TABLE>
  >]
...
#| out-width: 100%
#| echo: false
knitr::include_graphics("figs/osm_gsv_flowchart-5-steps.png")
```
...
# helper: kept in case you need presence later
...
safe_read_exact <- function(path, sheet_name){
  ...
  df %>%
    dplyr::mutate(
      ...
      # do not touch joined_result, keep whatever is there (TRUE/FALSE/1/0/NA)
      usable        = !is.na(joined_result)  # THIS is the key line
    )
}

```

<mark>Code requires adjustment to scale. This table should show usable points only.</mark>.
...
# -------- 2) READ sheets via the new safe_read_exact -------------------------
...
# -------- 4) USABLE rows (joined_result not NA) ------------------------------
usable_tbl <- bind_rows(add_tbl, rem_tbl, nonci_tbl) %>%
  filter(
    usable,              # defined in safe_read_exact as !is.na(joined_result)
    !is.na(joined_result) # redundant but explicit
  )
...
<mark>Code requires adjustment to scale. Calculations should be based on followup and baseline to estimate ADD and REMOVE False Negatives.</mark>.
...
# EXACT file you uploaded (fixed)
FILE <- "outputs/barcelona_samples_2015_2023_joined_results_20251103.xlsx"

# --- helper: normalise joined_result to TRUE / FALSE / NA --------------------
norm_bool <- function(x){
  y <- tolower(trimws(as.character(x)))
  dplyr::case_when(
    y %in% c("true", "1")  ~ TRUE,
    y %in% c("false", "0") ~ FALSE,
    TRUE                   ~ NA
  )
}

# --- reader: get cleaned joined_result for a sheet ---------------------------
read_joined_sheet <- function(path, sheet) {
  df <- tryCatch(
    readxl::read_xlsx(path, sheet = sheet, skip = 1, .name_repair = "minimal"),
    error = function(e) tibble::tibble()
  )
  if (!nrow(df)) return(tibble::tibble(joined_result = logical()))
  ...
  # 2) Drop summary rows if there is a tract_id column
  if ("tract_id" %in% names(df)) {
    df <- df %>% dplyr::filter(!is.na(tract_id))
  }

  # 3) Find the joined_result column
  jr_name <- dplyr::case_when(
    "joined_result"  %in% names(df) ~ "joined_result",
    "joined_results" %in% names(df) ~ "joined_results",
    TRUE ~ NA_character_
  )
  if (is.na(jr_name)) return(tibble::tibble(joined_result = logical()))

  df %>%
    dplyr::transmute(
      joined_result = norm_bool(.data[[jr_name]])
    ) %>%
    dplyr::filter(!is.na(joined_result))   # usable points only (no BANK / NA)
}

# ---------- read sheets (only joined_result) ---------------------------------
add_j <- read_joined_sheet(FILE, "ADD")
rem_j <- read_joined_sheet(FILE, "REMOVE")
gen_j <- read_joined_sheet(FILE, "NONCI")   # for false negatives

# ---------- Map joined_result to TP / FP / FN --------------------------------
# ADD (OSM says ADD)
n_add_usable <- nrow(add_j)
TP_add <- sum(add_j$joined_result == TRUE,  na.rm = TRUE)
FP_add <- sum(add_j$joined_result == FALSE, na.rm = TRUE)

# REMOVE (OSM says REMOVE)
n_rem_usable <- nrow(rem_j)
TP_rem <- sum(rem_j$joined_result == TRUE,  na.rm = TRUE)
FP_rem <- sum(rem_j$joined_result == FALSE, na.rm = TRUE)

# NONCI (OSM says NO CHANGE) -> false negatives (any missed change)
FN_T <- 0L
...
# Use the same FN pool (from NONCI) for both classes
FN_add <- FN_T
FN_rem <- FN_T
...
# ---------- Final table: ONE column for Precision, ONE for Recall ------------
t_class <- tibble(
