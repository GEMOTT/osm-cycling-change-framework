...
perimeter_path <- "data/processed/barcelona_perimeter.gpkg"
if (!file.exists(perimeter_path)) {
  stop("Perimeter file not found: ", perimeter_path,
       "\nRebuild it in the OSM download chunk or call read_or_build_perimeter(...).")
}

# Detect existing layer(s)
layers <- st_layers(perimeter_path)$name
stopifnot(length(layers) > 0)

# If our standard layer is absent, read the first one and rewrite with a fixed name
target_layer <- "barcelona_perimeter"
if (!(target_layer %in% layers)) {
  message("Standardising perimeter layer name to '", target_layer, "'.")
  tmp <- st_read(perimeter_path, layer = layers[1], quiet = TRUE)
  st_write(tmp, perimeter_path, layer = target_layer,
           driver = "GPKG", append = FALSE, quiet = TRUE)
}

# Read with the fixed name and ensure WGS84
city_perimeter <- st_read(perimeter_path, layer = target_layer, quiet = TRUE)
if (is.na(st_crs(city_perimeter)$epsg) || st_crs(city_perimeter)$epsg != 4326) {
...
fetch_and_crop_data("160101", city_perimeter, "data/processed/barcelona_15_lines.gpkg")
fetch_and_crop_data("240101", city_perimeter, "data/processed/barcelona_23_lines.gpkg")
...
openxlsx::setColWidths(wb, sheet, cols = 4:7, widths = 0)
  ...
  # --- header styling: highlight 2015 & 2023; grey 2018 & 2024 ----------------
