...
Urban transformations, such as new bike lanes, reshape mobility and health.

However, standardised datasets that track how infrastructure changes over time remain scarce.

**OpenStreetMap (OSM)** offers open, historical data but its **reliability for detecting change** is still uncertain.  

**Aim**: Assess whether OSM reliably reflects new or removed bike lanes in Barcelona (2019–2023) using **Google Street View (GSV)** as ground truth, providing a basis for scalable validation in other cities.
...
# This chunk runs first, prepares packages and a tiny on-disk cache.
  ...
  library(leaflet); library(htmlwidgets)             # interactive maps
  ...
  library(htmltools)    
                         # small HTML helpers for outputs
...
- 6 tracts per stratum, randomly selected
...
<!-- - Metrics reported by class and pooled (95 % CI) -->
...
<!-- ### S2. Interactive stratification map (density × centrality) with sampled tracts -->
...
<!--   sampled_ll <- sampled_tracts |> sf::st_make_valid() |> sf::st_transform(4326) -->
<!-- } -->
...
<!-- # Color matrix - ensure proper orientation -->
<!-- M <- matrix(cols, nrow = dim_bi, byrow = TRUE) -->

<!-- labs_df   <- expand.grid(y = 1:dim_bi, x = 1:dim_bi)  # y first for correct ordering -->
<!-- lab_names <- sprintf("%d-%d", labs_df$x, labs_df$y) -->
<!-- lab_cols  <- vapply(seq_len(nrow(labs_df)), function(i) M[labs_df$y[i], labs_df$x[i]], "") -->
...
<!-- # 4) CORRECTED: Add a clean bivariate legend with proper orientation -->
<!-- add_bi_legend_svg <- function(map, -->
<!--                               pal = "DkBlue2", dim = 3, -->
<!--                               reverse_x = FALSE,        # flip density if needed -->
<!--                               reverse_y = TRUE,         # keep purple = higher centrality -->
<!--                               position = "bottomright", -->
<!--                               title = "", -->
<!--                               xlab = "Higher Density →", -->
<!--                               ylab = "More Central →", -->
<!--                               box = 18, gap = 2, font_size = 8){ -->

<!--   cols <- biscale::bi_pal(pal, dim, preview = FALSE) -->
<!--   M    <- matrix(cols, nrow = dim, byrow = TRUE) -->
<!--   if (reverse_y) M <- M[dim:1, ] -->
<!--   if (reverse_x) M <- M[, dim:1] -->
...
<!--       y <- (i-1)*(box+gap) -->
<!--       rects <- c(rects, -->
<!--         sprintf('<rect x="%d" y="%d" width="%d" height="%d" fill="%s" stroke="#999" stroke-width="0.5"/>', -->
<!--                 x, y, box, box, M[i, j])) -->
<!--     } -->
<!--   } -->

<!--   svg <- sprintf(' -->
<!--   <div style="background:#fff;padding:6px 8px;border-radius:6px;box-shadow:0 1px 4px rgba(0,0,0,.3);font-family:sans-serif"> -->
<!--     %s -->
<!--     <div style="display:flex;align-items:center;gap:8px"> -->
<!--       <div style="writing-mode:vertical-rl;transform:rotate(180deg);font-size:%dpx;color:#444;">%s</div> -->
<!--       <svg xmlns="http://www.w3.org/2000/svg" width="%d" height="%d" viewBox="0 0 %d %d" style="display:block"> -->
<!--         %s -->
<!--       </svg> -->
<!--     </div> -->
<!--     <div style="text-align:center;font-size:%dpx;color:#444;margin-top:4px;">%s</div> -->
<!--   </div>', -->
<!--     if (nzchar(title)) sprintf('<div style="font-weight:600;margin-bottom:4px;font-size:%dpx">%s</div>', font_size, title) else "", -->
<!--     font_size, ylab, w, h, w, h, paste(rects, collapse=""), font_size, xlab -->
<!--   ) -->

<!--   leaflet::addControl(map, html = htmltools::HTML(svg), position = position) -->
<!-- } -->
...
<!-- # 5) Add the corrected bi-legend -->
<!-- m <- add_bi_legend_svg(m, pal = "DkBlue2", dim = 3, position = "bottomright") -->
<!-- m -->
...
<!-- # 5) Add the corrected bi-legend -->
<!-- m <- add_bi_legend_svg(m, pal = "DkBlue2", dim = 3, -->
<!--                        reverse_x = FALSE, reverse_y = TRUE,  # purple = central -->
<!--                        position = "bottomright", -->
<!--                        title = "") -->
...
<!-- dir.create("maps", showWarnings = FALSE) -->
<!-- saveWidget(m, "maps/stratification.html", selfcontained = TRUE)  -->
<!-- ``` -->

<!-- <iframe src="maps/stratification.html" width="100%" height="650" style="border:0" loading="lazy"></iframe> -->
...
prep_pts <- function(x){
  ...
  gsv <- sprintf(
    "https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=%f,%f",
    w$lat, w$lon
  )
  w$gsv <- utils::URLencode(gsv, reserved = TRUE)

  make_html <- function(i) HTML(paste0(
    "<b>", if ("class" %in% names(w)) w$class[i] else "Point", "</b>",
    if ("tract_id" %in% names(w)) paste0("<br/>tract: ", w$tract_id[i]) else "",
    if ("stratum"  %in% names(w)) paste0("<br/>stratum: ", w$stratum[i]) else "",
    "<br/><a href='", w$gsv[i], "' target='_blank' rel='noopener'>Open GSV</a>"
  ))
  w$popup_html <- lapply(seq_len(nrow(w)), make_html)
  w$popup <- vapply(w$popup_html, as.character, "", USE.NAMES = FALSE)
  w
}
...
# ---- bounds (prefer city_perimeter; include layers as fallback) -------------
get_bbox <- function(obj) if (inherits(obj,"sf") && nrow(obj)) sf::st_bbox(sf::st_transform(obj, 4326)) else NULL
...
if (exists("city_perimeter") && inherits(city_perimeter,"sf") && nrow(city_perimeter)) {
  ...
  if (!is.null(gen_wgs)  && nrow(gen_wgs))  bbs <- c(bbs, list(sf::st_bbox(gen_wgs)))
  if (inherits(gnet_wgs,"sf") && nrow(gnet_wgs)) bbs <- c(bbs, list(sf::st_bbox(gnet_wgs)))
  if (!is.null(pers_wgs) && nrow(pers_wgs)) bbs <- c(bbs, list(sf::st_bbox(pers_wgs)))
  if (!length(bbs) && exists("tracts") && inherits(tracts,"sf") && nrow(tracts)) {
    bbs <- list(get_bbox(tracts))
  }
  if (length(bbs)) {
...
if (exists("tracts") && inherits(tracts,"sf") && nrow(tracts)) {
  m <- m %>% addPolygons(
    ...
    fillColor  = tract_fill,  fillOpacity = 0.05,
    smoothFactor = 0.5,
    highlightOptions = highlightOptions(weight = 2, color = "#111827", fillOpacity = 0.1, bringToFront = TRUE),
    label = ~as.character(tract_id)
  )
  # Area labels (centroids)
...
# Points (use the list-column explicitly to avoid knit env quirks)
if (!is.null(add_wgs) && nrow(add_wgs) > 0)
  m <- m %>% addCircleMarkers(
    data = add_wgs, lng = ~lon, lat = ~lat, group = "ADD pts",
    radius = 4, weight = 1, color = stroke, fillColor = col_add_pt, fillOpacity = 0.95,
...
# Dynamic legend (only show series that exist)
leg_cols <- c(); leg_labs <- c()
if (!is.null(add_wgs)  && nrow(add_wgs)  > 0) { leg_cols <- c(leg_cols, col_add_pt);  leg_labs <- c(leg_labs, "ADD points") }
if (!is.null(rem_wgs)  && nrow(rem_wgs)  > 0) { leg_cols <- c(leg_cols, col_rem_pt);  leg_labs <- c(leg_labs, "REMOVE points") }
if (!is.null(gen_wgs)  && nrow(gen_wgs)  > 0) { leg_cols <- c(leg_cols, col_gen_pt);  leg_labs <- c(leg_labs, "NONCI points") }
if (length(leg_cols)) {
  m <- m %>% addLegend(position = "bottomright", title = "", colors = leg_cols, labels = leg_labs, opacity = 1)
}

m

# dir.create("maps", showWarnings = FALSE)
# saveWidget(m, "maps/ci_change.html", selfcontained = TRUE)
# )
```
-->
<!-- <iframe src="maps/ci_change.html" width="100%" height="650" style="border:0" loading="lazy"></iframe> -->

<!-- ```{r} -->
<!-- #| label: 28-validation-map-points-static-built -->
<!-- #| eval: false -->
<!-- #| include: false -->
<!-- # Requires: added_pts, removed_pts, nonci_pts, (optional) ci_static_pts OR ci_stable_pts, and sampled_tracts or tracts -->
...
<!-- if (!exists("nonci_pts"))     nonci_pts     <- NULL -->
<!-- # accept either name from upstream: ci_static_pts or ci_stable_pts -->
...
<!-- # colours (+ CI_STATIC) -->
<!-- cols <- c(ADD = "#E6AB02", REMOVE = "#D95F02", NONCI = "#1B9E77") -->
...
<!--     mins <- do.call(pmin, lapply(cands, function(b) c(b["xmin"], b["ymin"]))) -->
<!--     maxs <- do.call(pmax, lapply(cands, function(b) c(b["xmax"], b["ymax"]))) -->
<!--     # Make bb a NUMERIC vector, not a list: -->
<!--     bb <- c(xmin = as.numeric(mins[1]), -->
<!--             ymin = as.numeric(mins[2]), -->
<!--             xmax = as.numeric(maxs[1]), -->
<!--             ymax = as.numeric(maxs[2])) -->
...
<!--   } -->
<!-- } -->
<!-- bounds <- as.numeric(bb[c("xmin", "ymin", "xmax", "ymax")]); names(bounds) <- NULL -->
...
<!-- <iframe src="maps/infra_change.html" width="100%" height="650" style="border:0" loading="lazy"></iframe> -->
